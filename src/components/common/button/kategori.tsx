        ...
        <div className="absolute z-50 mt-2 w-full overflow-hidden rounded-2xl bg-cream-light shadow-lg  animate-in fade-in zoom-in duration-100 ">
          <div className="py-1">
            {categories.map((category) => (
              <button key={category.id} onClick={() => handleSelect(category)} className="block w-full px-5 py-3 text-left text-body text-cream-dark-active hover:bg-[#f7f3f0]  font-poppins transisi cursor-pointer">
                {category.name}
              </button>
            ))}
          </div>
