...
export default function SearchComponent({ onSearch, onSelectSuggestion }: SearchComponentProps) {
  ...
  return (
    <div ref={containerRef} className="relative rounded-2xl bg-cream-light w-full">
      ...
      {showSuggestions && (
        <ul className="absolute z-50 top-full left-0 right-0 mt-1 bg-(--cream) rounded-2xl shadow-md border border-gray-100 overflow-hidden">
          {loading ? (
            <li className="px-4 py-3 text-[12px] font-poppins text-gray-400">Mencari...</li>
          ) : (
            suggestions.map((item) => (
              <li key={item.id} onClick={() => handleSelect(item)} className="px-4 py-3 text-[13px] font-poppins text-dark cursor-pointer bg-(--cream) hover:bg-(--cream-hover) flex items-center gap-3 transition-colors">
